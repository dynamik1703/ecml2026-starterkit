...
def config_key(row: dict[str, Any]) -> tuple[float, float, float, int]:
    return (
        safe_float(row.get("planner_unsafe_weight")),
        safe_float(row.get("planner_reward_risk_weight")),
        safe_float(row.get("planner_score_threshold")),
...
def read_metrics(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise ValueError(f"{path} does not contain a metrics list")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare prefix-planner deployment summaries across OOD blocks and "
            "rank shared configs by aggregate safety and lift."
        )
    )
    parser.add_argument(
        "json",
...
def main() -> int:
    args = parse_args()
    blocks = [(name, read_metrics(path)) for name, path in map(named_path, args.json)]
